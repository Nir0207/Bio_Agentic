...
from graphML.writers.writeback import write_graph_embedding_metadata

logger = logging.getLogger(__name__)

FASTRP_WRITE_QUERY = """
CALL gds.fastRP.write($graphName, $config)
YIELD nodePropertiesWritten, computeMillis, writeMillis, preProcessingMillis
RETURN nodePropertiesWritten, computeMillis, writeMillis, preProcessingMillis
""".strip()
...
def run_fastrp(client: Neo4jClient, settings: Settings) -> FastRPResult:
    ...
    result = FastRPResult(
        node_properties_written=int(write_row.get("nodePropertiesWritten", 0)),
        metadata_rows_updated=metadata_rows,
        compute_millis=int(write_row.get("computeMillis", 0)),
        write_millis=int(write_row.get("writeMillis", 0)),
        pre_processing_millis=int(write_row.get("preProcessingMillis", 0)),
    )
    logger.info(
